...
interface LostItemDao {
    ...
    /**
     * Actualiza el estado de favorito de un item.
     * También limpia favoriteSyncedAt para que un worker pueda sincronizarlo más adelante.
     */
    @Query("""
        UPDATE lost_items_cache
        SET isFavorite = :isFavorite,
            favoriteUpdatedAt = :updatedAt,
            favoriteSyncedAt = NULL
        WHERE id = :itemId
    """)
    suspend fun updateFavorite(itemId: String, isFavorite: Boolean, updatedAt: Long)

    /**
     * Devuelve solo los ítems marcados como favoritos.
     */
    @Query("SELECT * FROM lost_items_cache WHERE isFavorite = 1 ORDER BY updatedAt DESC")
    suspend fun getFavorites(): List<LostItemEntity>

    /**
     * Observa en tiempo real los ítems favoritos (para una futura pantalla de 'Mis favoritos').
     */
    @Query("SELECT * FROM lost_items_cache WHERE isFavorite = 1 ORDER BY updatedAt DESC")
    fun observeFavorites(): Flow<List<LostItemEntity>>
}
